defmodule ShotDs.Stt.TermFactoryTest do
  ...
  describe "with_local_cleanup/1" do
    ...
    test "cleans up temporary fresh variables generated by the DSL lambda" do
      table_size_before = :ets.info(:term_pool, :size)

      final_id = lambda(Type.new(:i), fn x -> x end)

      table_size_after = :ets.info(:term_pool, :size)

      net_growth = table_size_after - table_size_before
      assert net_growth == 2
      assert %Term{} = TF.get_term(final_id)
    end
  end
